...
class Yolov5Evaluator:
    ...
    def run_training(self, model, dataloader, compute_loss=None):
        ...
        for batch_i, (img, targets, paths, shapes) in enumerate(
            ...
            out = self.inference(img, targets, compute_loss)
    ...
    def run(
        ...
        model, dataloader, imgsz = self.before_infer(weights, batch_size, task)
        self.half &= self.device.type != "cpu"  # half precision only supported on CUDA
        model.half() if self.half else model.float()
        # Configure
        model.eval()
        ...
        for batch_i, (img, targets, paths, shapes) in enumerate(
            ...
            out = self.inference(img, targets)
    ...
    def before_infer(self, weights, batch_size, imgsz, task="val"):
    ...
    def inference(self, img, targets, compute_loss=None):
        """Inference"""
        t1 = time_sync()
