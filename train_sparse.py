...
def compute_pruning_loss(p, prunable_modules, loss):
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    ll1 = ft([0])
    if prunable_modules is not None:
        for m in prunable_modules:
            # ll1 += m.weight.norm(1)
            ll1 += m.weight.pow(2).sum() / 2
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss = compute_pruning_loss(pred, prunable_modules, loss)
                if RANK != -1:
            ...
            # # not work when mix precision training
            # # --------------------sparse---------------------------------------
            # for k, m in model.named_modules():
            #     if isinstance(m, nn.BatchNorm2d):
            #         print('before:', m.weight.grad.data[0])
            #         m.weight.grad.data.add_(0.5 * torch.sign(m.weight.data))
            #         m.bias.grad.data.add_(0.5 * torch.sign(m.bias.data))
            #         print('after:', m.weight.grad.data[0])
            #         print('-------------')
            # # --------------------sparse---------------------------------------
