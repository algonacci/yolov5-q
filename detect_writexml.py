...
parser.add_argument('--output', type=str, default='/d/九江/playphone/1218-1221_label',
...
for img_name in imgs_list:
    print(os.path.join(source, img_name))
    ...
    if num == 1 and roi:
        for i in range(num_points):
            poly_points.append(
                cv2.selectROI(windowName="roi", img=img0, showCrosshair=False, fromCenter=False))
    ...
    if det is not None and len(det):
        ...
        for *xyxy, conf, cls in reversed(det):
            ...
            node_name_object.appendChild(doc.createTextNode(names[int(cls)]))
            ...
            with open(os.path.join(out, os.path.splitext(img_name)[0] + '.xml'), 'w') as fh:
                # 将根节点添加到文档对象中
                fh.truncate()
