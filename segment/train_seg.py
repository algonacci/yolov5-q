...
import val_seg  # for end-of-epoch mAP
...
from yolov5.utils.loggers import LoggersMask
...
def train(hyp, opt, device, callbacks):  # path/to/hyp.yaml or hyp dictionary
    ...
    # Loggers
    if RANK in [-1, 0]:
        loggers = LoggersMask(save_dir, weights, opt, hyp, LOGGER)  # loggers instance
        if loggers.wandb:
