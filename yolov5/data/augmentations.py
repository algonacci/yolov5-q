...
class RandomPerspective:
    ...
    def get_affine_matrix(self, img, border):
        ...
        return M, width, height, s

    def apply_affine_to_bboxes(self, bboxes, width, height, M):
        """apply affine to bboxes only.

        Args:
            bboxes(List[]): list of bboxes, [x1, y1, x2, y2...].
    ...
    def apply_affine_to_segments(self, segments, width, height, M):
        """apply affine to segments and generate new bboxes from segments.

        Args:
            segments(List[]): list of segments, [num_points, 2]*N.
            width(int): image width.
            height(int): image height.
            M(ndarray): affine matrix.
        Returns:
            new_segments(List[]): list of segments after affine, [num_points, 2]*N.
            new_bboxes(ndarray): bboxes after affine, [N, 4].
        """
        n = len(segments)
        if n == 0:
    ...
    def apply_affine_to_keypoints(self, keypoints, width, height, M):
        """apply affine to keypoints.

        Args:
            keypoints(ndarray): keypoints, (N, num_points, 2), [x, y, x, y...].
            width(int): image width.
            height(int): image height.
            M(ndarray): affine matrix.
        Return:
            new_keypoints(ndarray): keypoints after affine, (N, num_points, 2),
                [x, y, x, y...].
        """
        n = len(keypoints)
        if n == 0:
    ...
    def __call__(self, img, targets, border=(0, 0)):
        ...
        Args:
            img(ndarray): image.
            targets(Dict): a dict of `bboxes`, `segments`, `keypoints`.
            border(tuple): mosaic border.
        """
        # cls, x, y, w, h
        labels = targets.get("labels", [])
        if len(labels) == 0:
            return img, targets
        M, width, height, scale = self.get_affine_matrix(img, border)
        new_bboxes = self.apply_affine_to_bboxes(labels[:, 1:], width, height, M)
        i = box_candidates(
            box1=labels[:, 1:5].T * scale,
            box2=new_bboxes.T,
            cls=labels[:, 0],
        ...
        if len(segments):
            new_bboxes, new_segments = self.apply_affine_to_segments(
                segments, width, height, M
            )
            new_segments = np.array(new_segments)[i]
            targets["segments"] = new_segments
