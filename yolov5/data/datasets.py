...
class LoadImagesAndLabels(Dataset):
    ...
    def cache_images(self, cache_images, prefix):
        ...
        results = ThreadPool(NUM_THREADS).imap(
            lambda x: self.load_image(*x), zip(range(self.num_imgs))
        )
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img, labels = self.apply_affine(img, targets, self.mosaic_border)
            ...
            if self.augment:
                img, labels = self.apply_affine(img, {"labels": labels})
        ...
        for j, (i, index) in enumerate(zip(ri, indices)):
            ...
            if j < (4 - num_neg):
                if len(self.img_bg_files) and (random.uniform(0, 1) > 0.5):
                    img, _, (h, w), temp_label = self.load_bg_image(index)
                else:
                    img, _, (h, w) = self.load_image(index)
            else:
                img, _, (h, w) = self.load_neg_image(index)
            # place img in img4
            if j == 0:
                img4 = np.full(
...
class LoadImagesAndLabelsAndMasks(LoadImagesAndLabels):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            targets = {"labels": labels, "segments": segments}
            img, labels, segments = self.apply_affine(img, targets, self.mosaic_border)
        else:
            # Load image
            img, (h0, w0), (h, w) = self.load_image(index)
            ...
            if self.augment:
                targets = {"labels": labels, "segments": segments}
                img, labels, segments = self.apply_affine(img, targets)
    ...
    def apply_affine(self, img, targets, border=(0, 0)):
        """Affine transformation"""
        img, targets = self.random_perspective(img, targets, border)
        return img, targets["labels"], targets["segments"]
