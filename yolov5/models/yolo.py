...
class DetectSegment(Detect):
    def __init__(
        self, nc=80, anchors=(), mask_dim=32, proto_channel=256, ch=(), inplace=True
    ):
    ...
    def forward(self, x):
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            return (
                (x, proto_out) if self.training else (torch.cat(z, 1), (x, proto_out))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg", type=str, default="yolov5n_seg.yaml", help="model.yaml"
    )
    parser.add_argument(
        "--device", default="", help="cuda device, i.e. 0 or 0,1,2,3 or cpu"
    )
    parser.add_argument(
        "--profile", action="store_true", default=True, help="profile model speed"
    )
